    ...
    <div className="bg-white rounded-2xl shadow-lg p-8">
      ...
      {createdForm && getFormId() && (
        <>
          ...
          <div className="mt-6">
            <h3 className="text-lg font-semibold mb-3">Danh sách Chi tiết Câu hỏi:</h3>
              ...
              {createdForm.questions.map((question, index) => (
                ...
                >
                  <h4 className="text-base font-bold text-indigo-700 mb-2">
                    {index + 1}. {question.naturalQuestion || 'Không có câu hỏi tự nhiên'}
                  </h4>
                  
                  {/* Loại câu hỏi */}
                  <p className="text-sm text-gray-600 mb-1">
                    <span className="font-semibold">Loại (Type):</span> 
                    <code className="bg-blue-100 text-blue-800 px-2 py-0.5 rounded-md ml-1">{question.type}</code>
                  </p>

                  {/* Câu hỏi gốc (OriginalQuestion) */}
                  <p className="text-sm text-gray-600 mb-1">
                    <span className="font-semibold">Câu hỏi Gốc:</span> {question.originalQuestion || 'Không có câu hỏi gốc'}
                  </p>

                  {/* Các lựa chọn (Options) */}
                  {question.options && question.options.length > 0 && (
                    <div className="mt-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
                      <p className="font-semibold text-sm mb-1 text-gray-700">Các Lựa chọn (Options):</p>
